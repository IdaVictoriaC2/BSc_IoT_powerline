...
class GapRecoveryService:
    ...
    def check_pending_gaps_after_insert(self, client) -> None:
        for gap in self.database.get_pending_gaps():
            if self.is_gap_filled(gap["device_eui"], gap["start_ts"], gap["end_ts"]):
                self.database.log_event(
                    "RECOVERY_COMPLETED",
                    f"Recovery interval {gap['start_ts']}-{gap['end_ts']} completed for {gap['device_eui']}.",
                )
                with self.pending_gaps_lock:
                    self.database.delete_pending_gap(gap["device_eui"])
    ...
    def recovery_retry_loop(self, client) -> None:
        while True:
            ...
            for gap in self.database.get_pending_gaps():
                ...
                if now - float(gap.get("last_requested_at", 0)) < self.config.recovery_retry_after_seconds:
                    continue
                ...
                if retry_count >= self.config.max_recovery_retries:
                    self.database.log_event(
                        "RECOVERY_FAILED",
                        f"Recovery failed for {gap['device_eui']}: interval {gap['start_ts']}-{gap['end_ts']} was not filled after {self.config.max_recovery_retries} retries.",
                    )
                    with self.pending_gaps_lock:
                        self.database.delete_pending_gap(gap["device_eui"])
                    continue

                self.send_retransmission_request(client, gap["app_id"], gap["device_eui"], gap["start_ts"], gap["end_ts"])
                with self.pending_gaps_lock:
                    self.database.upsert_pending_gap(
